package com.taotaosou.lu.redis.demo;
...
//                LOGGER.error("{}正被{}锁定.", key, desc);
            ...
            Thread.sleep(100);
             compare=(System.nanoTime() - begin) - unit.toNanos(timeout);
        ...
        executor.execute(new Runnable()
        {
            public void run()
            {
                try
                {
                    ...
                    boolean isLock = DistributedLockManger.tryLock(key, value, 5000, TimeUnit.MILLISECONDS);
                    if(k<=0)return;
                    if (isLock){
...
public static void main(String[] args) {
	System.out.println(TimeUnit.MILLISECONDS.toMillis(500));
}
